...
const listarUsuarios = async (page) => {
    ...
    if( next === null) {
        document.querySelector(".numero_pagina").textContent = first; // numero de pagina
    }else {
        document.querySelector(".numero_pagina").textContent = next - 1; // numero de pagina
    }

    //nav.querySelector('.first').setAttribute("disabled", "");
    //nav.querySelector('.pre').setAttribute("disabled", "");
    ...
    nav.querySelector('.prev').setAttribute("data-pre", prev);
...
document.addEventListener("click", (event) => {
    ...
    if(event.target.matches(".first")){
        ...
        while(nodos.firstChild) {
            nodos.removeChild(nodos.firstChild); // elimina todos los tr
        }
